...
class TokenCalculator:
    ...
    def estimate_tokens(self, 
                       text: str, 
                       model: str = "gpt-4",
                       max_completion_tokens: int = 1000) -> TokenEstimate:
        """
    ...
    def analyze_prompt_complexity(self, text: str) -> Dict[str, any]:
        ...
        if analysis["word_count"] > 1000:
            ...
            analysis["suggestions"].append("Consider breaking into smaller prompts")
        elif analysis["word_count"] > 500:
            analysis["complexity"] = "high"
            analysis["suggestions"].append("Large prompt may be expensive")
        ...
        if len(words) != len(set(words)) and len(words) > 20:
            ...
            if repetition_ratio > 0.3:
                analysis["suggestions"].append("High repetition detected - consider consolidating")
        
        # Check for very long lines
        long_lines = [line for line in text.splitlines() if len(line) > 200]
        if long_lines:
            analysis["suggestions"].append(f"{len(long_lines)} very long line(s) detected")
