...
class PromptDataManager:
    def __init__(self, db_path: str = None, tenant_id: str = None, user_id: str = None):
    ...
    def init_database(self):
        ...
        if self.db_type == "postgres":
            ...
            if 'id' not in config_columns:
                ...
                ''')
                cursor.execute('INSERT INTO config_new (key, value) SELECT key, value FROM config')
    ...
    def add_prompt(self, name: str, title: str, content: str, category: str, tags: str, is_enhancement_prompt: bool = False) -> str:
        ...
        if self.db_type == "postgres":
            cursor.execute('SELECT id FROM prompts WHERE name = %s AND tenant_id = %s', (name, self.tenant_id))
        else:
            cursor.execute('SELECT id FROM prompts WHERE name = ? AND tenant_id = ?', (name, self.tenant_id))
        
        if cursor.fetchone():
            conn.close()
            return f"Error: A prompt with name '{name}' already exists in your workspace!"
        
        if self.db_type == "postgres":
            cursor.execute('''
                INSERT INTO prompts (tenant_id, user_id, name, title, content, category, tags, is_enhancement_prompt, created_at, updated_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ''', (self.tenant_id, self.user_id, name, title.strip(), content.strip(), category, tags.strip(), is_enhancement_prompt,
    ...
    def update_prompt(self, original_name: str, new_name: str, title: str, content: str, category: str, tags: str, is_enhancement_prompt: bool = False) -> str:
        ...
        if self.db_type == "postgres":
            cursor.execute('SELECT id FROM prompts WHERE name = %s AND tenant_id = %s', (original_name, self.tenant_id))
        else:
            cursor.execute('SELECT id FROM prompts WHERE name = ? AND tenant_id = ?', (original_name, self.tenant_id))
        ...
        if original_name != new_name:
            if self.db_type == "postgres":
                cursor.execute('SELECT id FROM prompts WHERE name = %s AND tenant_id = %s', (new_name, self.tenant_id))
            else:
                cursor.execute('SELECT id FROM prompts WHERE name = ? AND tenant_id = ?', (new_name, self.tenant_id))
        ...
        if self.db_type == "postgres":
            ...
            ''', (new_name, title.strip(), content.strip(), category, tags.strip(), is_enhancement_prompt,
                  datetime.now(), original_name, self.tenant_id))
        else:
            cursor.execute('''
                UPDATE prompts 
            ...
            ''', (new_name, title.strip(), content.strip(), category, tags.strip(), is_enhancement_prompt,
                  datetime.now().isoformat(), original_name, self.tenant_id))
    ...
    def delete_prompt(self, name: str) -> str:
        ...
        if self.db_type == "postgres":
            cursor.execute('DELETE FROM prompts WHERE name = %s AND tenant_id = %s', (name.strip(), self.tenant_id))
        else:
            cursor.execute('DELETE FROM prompts WHERE name = ? AND tenant_id = ?', (name.strip(), self.tenant_id))
    ...
    def get_categories(self) -> List[str]:
        ...
        if self.db_type == "postgres":
            cursor.execute('SELECT DISTINCT category FROM prompts WHERE tenant_id = %s ORDER BY category', (self.tenant_id,))
        else:
            cursor.execute('SELECT DISTINCT category FROM prompts WHERE tenant_id = ? ORDER BY category', (self.tenant_id,))
        ...
        if self.db_type == "postgres":
            ...
            if include_enhancement_prompts:
                cursor.execute('''
                    ...
                    ORDER BY category, name
                ''', (self.tenant_id, f'%{search_term}%', f'%{search_term}%', f'%{search_term}%', f'%{search_term}%'))
            else:
                cursor.execute('''
                    SELECT id, tenant_id, user_id, name, title, content, category, tags, is_enhancement_prompt, created_at, updated_at
                    FROM prompts 
