...
Quick Start Examples:
  ...
  python run.py --port 8080               # Custom port
    ...
    parser.add_argument(
        "--multi-tenant", 
        action="store_true", 
        help="Enable multi-tenant mode (default)"
    )
    parser.add_argument(
        "--with-api", 
        action="store_true", 
    ...
    parser.add_argument(
        "--host", 
        default=None, 
        help="Server host (overrides SERVER_HOST)"
    )
    parser.add_argument(
        "--port", 
        type=int, 
        default=None, 
        help="Server port (overrides SERVER_PORT)"
    )
...
def get_configuration(args):
    ...
    if args.single_user:
        ...
        config['multitenant_mode'] = os.getenv("MULTITENANT_MODE", "true").lower() == "true"
    
    # API configuration
    if args.with_api:
        config['enable_api'] = True
    else:
        config['enable_api'] = os.getenv("ENABLE_API", "false").lower() == "true"
    
    # Server configuration (args override env vars)
    config['host'] = args.host or os.getenv("SERVER_HOST", "0.0.0.0")
    config['port'] = args.port or int(os.getenv("SERVER_PORT", 7860))
    
    # Other options
    config['debug'] = args.debug or os.getenv("DEBUG", "false").lower() == "true"
    config['share'] = args.share or os.getenv("GRADIO_SHARE", "false").lower() == "true"
    
    # Database configuration
    config['db_type'] = os.getenv("DB_TYPE", "sqlite").lower()
    config['db_path'] = os.getenv("DB_PATH", "prompts.db")
    config['postgres_dsn'] = os.getenv("POSTGRES_DSN")
    
    # Development mode
    config['local_dev_mode'] = os.getenv("LOCAL_DEV_MODE", "true").lower() == "true"
...
def display_startup_info(config):
    ...
    if config['enable_api']:
        ...
        print(f"  🔍 API Explorer: http://{config['host']}:{config['port']}/api/redoc")
    
    # Database information
    print(f"  💾 Database: {config['db_type'].upper()}")
    ...
    if config['enable_api']:
        ...
        print(f"  • API Reference: http://{config['host']}:{config['port']}/api/redoc")
    ...
    if config['multitenant_mode']:
        ...
        if config['enable_api']:
            print("  4. Create API tokens in Account Settings for programmatic access")
    else:
        print("🚀 Getting Started:")
        print("  1. Open the web interface")
...
def main():
    ...
    if config['enable_api']:
        try:
            ...
            print(f"✅ API endpoints added successfully ({len(api_manager.app.routes)} routes)")
