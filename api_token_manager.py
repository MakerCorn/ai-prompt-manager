...
class APITokenManager:
    ...
    def create_api_token(self, user_id: str, tenant_id: str, name: str, 
                        expires_days: Optional[int] = None) -> Tuple[bool, str, Optional[str]]:
        """
        ...
        try:
            ...
            if self.db_type == "postgres":
                ...
                ''', (token_id, user_id, tenant_id, name.strip(), token_prefix, token_hash, expires_at))
            else:
                cursor.execute('''
                    INSERT INTO api_tokens (id, user_id, tenant_id, name, token_prefix, token_hash, expires_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (token_id, user_id, tenant_id, name.strip(), token_prefix, token_hash, 
            ...
            return True, f"API token '{name}' created successfully{expiry_msg}", full_token
            
        except Exception as e:
            conn.close()
            return False, f"Error creating token: {str(e)}", None

    def validate_api_token(self, token: str) -> Tuple[bool, Optional[str], Optional[str]]:
        """
        ...
        try:
            ...
            if result:
                if self.db_type == "postgres":
                    user_id, tenant_id, token_id = result['user_id'], result['tenant_id'], result['id']
                else:
