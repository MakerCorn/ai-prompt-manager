...
class PromptRequest(BaseModel):
    ...
    category: str = Field(..., min_length=1, max_length=100, description="Prompt category")
    description: Optional[str] = Field(None, max_length=1000, description="Prompt description")
    tags: Optional[str] = Field(None, description="Comma-separated tags")
    visibility: str = Field("private", pattern="^(private|public)$", description="Prompt visibility")
    is_enhancement_prompt: bool = Field(False, description="Whether this is an enhancement prompt")
...
def create_prompt_router(db_path: str = "prompts.db") -> APIRouter:
    ...
    def get_data_manager_configured(
        ...
        # For now, return a basic data manager
        # In a real implementation, this would extract user context from the token
        return PromptDataManager(tenant_id="default", user_id="default", db_path=db_path)
    ...
    async def list_prompts(
        ...
        visibility: Optional[str] = Query(None, description="Filter by visibility (private|public|mine|all)"),
        search: Optional[str] = Query(None, description="Search in name, content, description"),
        include_enhancement_prompts: bool = Query(True, description="Include enhancement prompts"),
        include_public_from_tenant: bool = Query(True, description="Include public prompts from tenant"),
        ...
        try:
            ...
            if visibility == "public":
                prompts = data_manager.get_public_prompts_in_tenant(include_enhancement_prompts)
            elif visibility == "private":
                # Get only user's private prompts
                all_user_prompts = data_manager.get_all_prompts(include_enhancement_prompts)
                prompts = [p for p in all_user_prompts if p.get("visibility", "private") == "private"]
    ...
    async def get_visibility_stats(
        ...
        try:
            ...
            stats = {
                ...
                "private": len([p for p in all_prompts if p.get("visibility", "private") == "private"]),
                "public": len([p for p in all_prompts if p.get("visibility", "private") == "public"]),
                "by_category": {},
            }
            ...
            for prompt in all_prompts:
                ...
                if category not in stats["by_category"]:
                    stats["by_category"][category] = {"total": 0, "private": 0, "public": 0}
            ...
            raise HTTPException(status_code=500, detail="Failed to retrieve visibility statistics")
