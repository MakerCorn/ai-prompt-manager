...
class TestProjectCoreFunctionality:
    ...
    def test_add_project_duplicate_name_fails(self, data_manager):
        ...
        data_manager.add_project(
            name="duplicate-test", title="First Project", description="First project"
        )

        # Try to create second project with same name
        result = data_manager.add_project(
            name="duplicate-test", title="Second Project", description="Second project"
        )
...
class TestProjectOwnershipTransfer:
    ...
    def test_transfer_ownership_to_non_member_fails(
        self, data_manager, project_with_member
    ):
    ...
    def test_transfer_ownership_non_existent_project_fails(
...
class TestProjectSearch:
    ...
    def test_search_projects_by_type(self, data_manager):
        ...
        data_manager.add_project(
            name="general-proj", title="General Project", project_type="general"
        )
        data_manager.add_project(
            name="sequenced-proj", title="Sequenced Project", project_type="sequenced"
        )
    ...
    def test_search_projects_by_tags(self, data_manager):
        ...
        result = data_manager.add_project(name="tagged-proj", title="Tagged Project")
    ...
    def test_search_projects_with_pagination(self, data_manager):
        ...
        for i in range(5):
            data_manager.add_project(
                name=f"paginated-proj-{i}", title=f"Paginated Project {i}"
            )

        # Test pagination
        page1_results = data_manager.search_projects(
            search_term="paginated", limit=3, offset=0
        )
        page2_results = data_manager.search_projects(
            search_term="paginated", limit=3, offset=3
        )
    ...
    def test_search_projects_combined_filters(self, data_manager):
        ...
        combined_results = data_manager.search_projects(
            search_term="multi", project_type="developer", tags="filter"
        )

        assert len(combined_results) >= 1
        found_project = next(
            p for p in combined_results if p["name"] == "multi-filter-test"
        )
