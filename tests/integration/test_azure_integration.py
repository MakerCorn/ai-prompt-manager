...
def test_azure_token_calculator():
    ...
    try:
        ...
        for model in azure_models:
            try:
                estimate = calculator.estimate_tokens(
                    test_text, model, max_completion_tokens=500
                )
...
def test_entra_id_configuration():
    ...
    try:
        ...
        if entra_enabled:
            ...
            if login_url:
                print("  ✅ Entra ID login URL generated successfully")
            else:
                print("  ⚠️  Entra ID login URL generation failed (check configuration)")
        else:
            print("  ℹ️  Entra ID not enabled - set ENTRA_ID_ENABLED=true to test")
...
def print_environment_info():
    ...
    for var in azure_vars:
        ...
        if "SECRET" in var or "KEY" in var:
            ...
            value = (
                f"{value[:8]}..." if value != "Not set" and len(value) > 8 else value
            )
