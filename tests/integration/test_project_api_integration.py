    ...
    try:
        ...
        for prompt_data in prompts_data:
            ...
            response = requests.post(
                f"{api_url.replace(':' + api_url.split(':')[-1], ':' + str(int(api_url.split(':')[-1]) - 1))}/prompts/new",
                data=form_data,
                timeout=5,
            )
        ...
        for rule_data in rules_data:
            ...
            response = requests.post(
                f"{api_url.replace(':' + api_url.split(':')[-1], ':' + str(int(api_url.split(':')[-1]) - 1))}/rules/new",
                data=form_data,
                timeout=5,
            )
...
def test_project_crud_operations(api_url: str) -> bool:
    ...
    try:
        ...
        response = requests.post(
            f"{web_url}/projects/new", data=project_data, timeout=10
        )
...
def test_project_ownership_transfer(api_url: str) -> bool:
    ...
    try:
        ...
        response = requests.post(
            f"{api_url}/projects/999999/transfer-ownership",
            data=transfer_data,
            timeout=10,
        )
...
def test_project_tags_functionality(api_url: str) -> bool:
    ...
    try:
        ...
        response = requests.put(
            f"{api_url}/projects/999999/tags", json=tags_data, timeout=10
        )
...
def run_project_api_integration_tests(
    base_port: int = None, api_port: int = None
) -> bool:
