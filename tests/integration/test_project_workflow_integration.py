...
def start_single_user_server() -> Tuple[Optional[subprocess.Popen], Optional[int], Optional[int]]:
            ...
            if startup_time > 5:
                try:
                    ...
                    if response.status_code == 200:
                        print(f"✅ Single-user server started after {startup_time} seconds")
...
def create_test_project(session: Session, base_url: str) -> Optional[int]:
    ...
    try:
        ...
        # Create project
        response = session.post(f"{base_url}/projects/new", data=project_data, timeout=10)
        ...
        # Extract project ID
        project_id = extract_project_id_from_html(response.text, "workflow-test-project")
...
def create_test_content(session: Session, base_url: str) -> Dict:
    ...
    try:
        ...
        prompts_data = [
            {
                ...
                "content": "This is test content for workflow prompt 1. " + "A" * 200,  # Adds to token count
                "category": "Testing",
            ...
            },
            {
                ...
                "content": "This is test content for workflow prompt 2. " + "B" * 300,  # Adds to token count
        ...
        for prompt_data in prompts_data:
            response = session.post(f"{base_url}/prompts/new", data=prompt_data, timeout=10)
...
def test_project_token_cost_workflow(api_url: str, project_id: int) -> bool:
    ...
    try:
        ...
        response = requests.get(f"{api_url}/api/projects/{project_id}/token-cost", timeout=10)
        ...
        if response.status_code == 200:
            try:
                ...
                if cost_data.get("success"):
                    ...
                    required_fields = ['total_tokens', 'prompt_tokens', 'rule_tokens', 'total_cost']
                    ...
                    print(f"❌ Token cost API returned error: {cost_data.get('error', 'Unknown')}")
                    return False
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                response = requests.put(f"{api_url}/api/projects/{project_id}/tags", 
                    ...
                    if update_result.get("success"):
                        ...
                        if response.status_code == 200:
                            ...
                            if set(project_tags) == set(new_tags.split(',')):
                                ...
                                print(f"❌ Tags not updated correctly. Expected: {new_tags.split(',')}, Got: {project_tags}")
                                return False
                    else:
...
def test_complete_project_workflow(base_url: str, api_url: str) -> bool:
    ...
    try:
        ...
        response = requests.get(f"{api_url}/api/projects/{project_id}/permissions", timeout=10)
