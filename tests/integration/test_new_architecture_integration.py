...
class TestNewArchitectureIntegration:
    ...
    def setup_method(self):
        ...
        self.config = DatabaseConfig(
            db_type=DatabaseType.SQLITE, db_path=self.temp_db.name
        )
    ...
    def test_complete_user_management_workflow(self):
        ...
        tenant = Tenant(name="Test Organization", subdomain="test-org", max_users=50)
        ...
        self.logger.info(f"Created tenant {saved_tenant.id}: {saved_tenant.name}")
        ...
        user = User(
            tenant_id=saved_tenant.id or "default-tenant",
        ...
        self.logger.info(f"Created user {saved_user.id}: {saved_user.email}")

        # Step 3: Verify password authentication
        is_valid = self.password_handler.verify_password(
            password, saved_user.password_hash, saved_user.salt
        )
        assert is_valid is True

        # Verify wrong password fails
        is_invalid = self.password_handler.verify_password(
            "wrong_password", saved_user.password_hash, saved_user.salt
        )
        ...
        found_user = self.user_manager.find_user_by_email(
            saved_tenant.id, "admin@test-org.com"
        )
        ...
        self.logger.info(f"User login updated for user {updated_user.id}")
    ...
    def test_tenant_isolation(self):
        ...
        user1 = User(
            tenant_id=saved_tenant1.id or "tenant1",
        ...
        user2 = User(
            tenant_id=saved_tenant2.id or "tenant2",
        ...
        found_user1 = self.user_manager.find_user_by_email(
            saved_tenant1.id or "tenant1", "user@example.com"
        )
        found_user2 = self.user_manager.find_user_by_email(
            saved_tenant2.id or "tenant2", "user@example.com"
        )
    ...
    def test_logging_integration(self):
        ...
        from src.utils.logging_config import AuditLogger, StructuredLogger
        ...
        audit_logger.log_security_event(
            event_type="password_change", severity="INFO", user_id="test-user"
        )
