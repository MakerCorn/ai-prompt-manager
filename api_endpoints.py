...
class APIManager:
    ...
    async def get_current_user(self, credentials: HTTPAuthorizationCredentials = Depends(security)) -> UserInfo:
        ...
        # Get user details
        users = self.auth_manager.get_tenant_users(tenant_id)
    ...
    def setup_routes(self):
        ...
        async def get_user_info(user_info: UserInfo = Depends(self.get_current_user)):
            ...
            return APIResponse(
                ...
                data={
                    "user": user_info.dict(),
                    "token_stats": self.token_manager.get_token_stats(user_info.user_id)
                }
            )
        ...
        async def list_prompts(
            ...
            search: Optional[str] = Query(None, description="Search in name, title, content"),
            include_enhancement: bool = Query(True, description="Include enhancement prompts")
        ):
            ...
            if search:
                ...
                prompts = data_manager.get_prompts_by_category(category, include_enhancement)
            else:
        ...
        @self.app.get("/api/prompts/category/{category_name}", response_model=PromptListResponse)
        async def get_prompts_by_category(
            ...
            data_manager = self.get_data_manager(user_info)
            prompts = data_manager.get_prompts_by_category(category_name, include_enhancement)
        ...
        async def get_stats(user_info: UserInfo = Depends(self.get_current_user)):
            ...
            for category in categories:
                category_prompts = [p for p in regular_prompts if p['category'] == category]
                stats["category_breakdown"][category] = len(category_prompts)
