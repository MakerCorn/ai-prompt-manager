...
class BaseRepository(ABC, Generic[T]):
    ...
    def find_by_id(self, entity_id: Any) -> Optional[T]:
        ...
        try:
            ...
            self.logger.error(f"Error finding {self.table_name} by ID {entity_id}: {str(e)}")
    ...
    def find_all(
        ...
        try:
            ...
            raise DatabaseException(f"Failed to find {self.table_name} records: {str(e)}")
    ...
    def delete(self, entity_id: Any) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error deleting {self.table_name} ID {entity_id}: {str(e)}")
    ...
    def count(self, filters: Optional[Dict[str, Any]] = None) -> int:
        ...
        try:
            ...
            raise DatabaseException(f"Failed to count {self.table_name} records: {str(e)}")
    ...
    def _insert_entity(self, entity_dict: Dict[str, Any]) -> T:
        ...
        if self.db_manager.config.db_type.value == "postgres":
            ...
            if id_field in entity_dict and entity_dict[id_field]:
                # Use provided ID
                return self.find_by_id(entity_dict[id_field])
            else:
    ...
    def _update_entity(self, entity_dict: Dict[str, Any]) -> T:
        ...
        update_fields = [k for k in entity_dict.keys() if k != id_field and entity_dict[k] is not None]
        ...
        if affected_rows == 0:
            raise DatabaseException(f"No {self.table_name} found with {id_field} = {entity_id}")
...
class TenantAwareRepository(BaseRepository[T]):
    ...
    def _ensure_tenant_context(self) -> None:
        ...
        if not self.current_tenant_id:
            raise DatabaseException("Tenant context not set - call set_tenant_context() first")
    
    def _add_tenant_filter(self, filters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
    ...
    def find_by_id(self, entity_id: Any) -> Optional[T]:
        ...
        try:
            ...
            row = self.db_manager.execute_query(
                query, 
                (entity_id, self.current_tenant_id), 
                fetch_one=True
            )
            ...
            self.logger.error(f"Error finding tenant {self.table_name} by ID {entity_id}: {str(e)}")
            raise DatabaseException(f"Failed to find {self.table_name} by ID: {str(e)}")
    ...
    def delete(self, entity_id: Any) -> bool:
        ...
        try:
            ...
            id_field = self._get_id_field()
            query = f"DELETE FROM {self.table_name} WHERE {id_field} = ? AND tenant_id = ?"
            ...
            self.logger.error(f"Error deleting tenant {self.table_name} ID {entity_id}: {str(e)}")
            raise DatabaseException(f"Failed to delete {self.table_name}: {str(e)}")
