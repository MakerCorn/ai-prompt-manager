...
from ..exceptions.base import DatabaseException, ConfigurationException
...
class BaseDatabaseManager(ABC):
    ...
    def column_exists(self, table_name: str, column_name: str) -> bool:
        ...
        try:
            if self.config.db_type == DatabaseType.POSTGRES:
                ...
                """
                result = self.execute_query(query, (table_name, column_name), fetch_one=True)
                return result['exists'] if result else False
            else:
    ...
    def get_database_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            'path': self.config.db_path if self.config.db_type == DatabaseType.SQLITE else None,
            'dsn': self.config.dsn if self.config.db_type == DatabaseType.POSTGRES else None
        }
