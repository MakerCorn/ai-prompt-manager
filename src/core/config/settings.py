...
class DatabaseConfig:
    ...
    def validate(self) -> None:
        ...
        if self.db_type == DatabaseType.POSTGRES and not self.dsn:
            raise ConfigurationException("POSTGRES_DSN is required when using PostgreSQL")
...
class AuthConfig:
    ...
    def validate(self) -> None:
        ...
        if self.sso_enabled:
            ...
            if missing_fields:
                raise ConfigurationException(f"SSO enabled but missing: {', '.join(missing_fields)}")
        ...
        if self.entra_id_enabled:
            ...
            if missing_fields:
                raise ConfigurationException(f"Entra ID enabled but missing: {', '.join(missing_fields)}")
