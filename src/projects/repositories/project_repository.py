...
class ProjectRepository(TenantAwareRepository[Project]):
    ...
    def find_accessible_projects(self, user_id: str) -> List[Project]:
        ...
        try:
            ...
            self.logger.error(f"Error finding accessible projects for user {user_id}: {str(e)}")
            raise
...
class ProjectMemberRepository(TenantAwareRepository[ProjectMember]):
    ...
    def find_by_project(self, project_id: int) -> List[ProjectMember]:
        ...
        try:
            ...
            self.logger.error(f"Error finding members for project {project_id}: {str(e)}")
            raise
    ...
    def find_member(self, project_id: int, user_id: str) -> Optional[ProjectMember]:
        ...
        try:
            ...
            self.logger.error(f"Error finding member {user_id} in project {project_id}: {str(e)}")
            raise
    
    def get_user_role(self, project_id: int, user_id: str) -> Optional[ProjectMemberRole]:
    ...
    def remove_member(self, project_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error removing member {user_id} from project {project_id}: {str(e)}")
            raise
        ...
        if self.db_manager.config.db_type.value == "postgres":
            ...
            with self.db_manager.get_connection_context() as conn:
                ...
                if row:
                    ...
                    raise Exception(f"Failed to retrieve inserted project member with ID {last_id}")
    ...
    def _update_member(self, entity_dict: Dict[str, Any]) -> ProjectMember:
        ...
        # Build UPDATE query
        update_fields = [k for k in entity_dict.keys() if k != "id" and entity_dict[k] is not None]
...
class ProjectPromptRepository(TenantAwareRepository[ProjectPrompt]):
    ...
    def find_by_project(self, project_id: int) -> List[ProjectPrompt]:
        ...
        try:
            ...
            self.logger.error(f"Error finding prompts for project {project_id}: {str(e)}")
            raise
    ...
    def find_by_prompt(self, prompt_id: int) -> List[ProjectPrompt]:
        ...
        try:
            ...
            self.logger.error(f"Error finding projects for prompt {prompt_id}: {str(e)}")
            raise
    
    def find_association(self, project_id: int, prompt_id: int) -> Optional[ProjectPrompt]:
        ...
        try:
            ...
            self.logger.error(f"Error finding association project {project_id} - prompt {prompt_id}: {str(e)}")
            raise
    ...
    def remove_association(self, project_id: int, prompt_id: int) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error removing association project {project_id} - prompt {prompt_id}: {str(e)}")
            raise
    ...
    def _association_exists(self, association_id: int) -> bool:
        ...
        try:
            ...
            result = self.db_manager.execute_query(query, (association_id,), fetch_one=True)
            return result is not None
        ...
        if self.db_manager.config.db_type.value == "postgres":
            ...
            with self.db_manager.get_connection_context() as conn:
                ...
                if row:
                    ...
                    raise Exception(f"Failed to retrieve inserted project prompt with ID {last_id}")
    ...
    def _update_association(self, entity_dict: Dict[str, Any]) -> ProjectPrompt:
        ...
        row = self.db_manager.execute_query(fetch_query, (association_id,), fetch_one=True)
        if not row:
...
class ProjectRuleRepository(TenantAwareRepository[ProjectRule]):
    ...
    def find_by_rule_set(self, project_id: int, rule_set_name: str) -> List[ProjectRule]:
        ...
        try:
            ...
            self.logger.error(f"Error finding rule set {rule_set_name} for project {project_id}: {str(e)}")
            raise
    ...
    def find_association(self, project_id: int, rule_id: int) -> Optional[ProjectRule]:
        ...
        try:
            ...
            self.logger.error(f"Error finding association project {project_id} - rule {rule_id}: {str(e)}")
            raise
    ...
    def remove_association(self, project_id: int, rule_id: int) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error removing association project {project_id} - rule {rule_id}: {str(e)}")
            raise
        ...
        if self.db_manager.config.db_type.value == "postgres":
            ...
            with self.db_manager.get_connection_context() as conn:
                ...
                if row:
                    ...
                    raise Exception(f"Failed to retrieve inserted project rule with ID {last_id}")
...
class ProjectVersionRepository(TenantAwareRepository[ProjectVersion]):
    ...
    def find_by_project(self, project_id: int) -> List[ProjectVersion]:
        ...
        try:
            ...
            self.logger.error(f"Error finding versions for project {project_id}: {str(e)}")
            raise
    
    def find_by_version_number(self, project_id: int, version_number: int) -> Optional[ProjectVersion]:
        ...
        try:
            ...
            self.logger.error(f"Error finding version {version_number} for project {project_id}: {str(e)}")
            raise
    ...
    def _insert_version(self, entity_dict: Dict[str, Any]) -> ProjectVersion:
        ...
        if self.db_manager.config.db_type.value == "postgres":
            ...
            with self.db_manager.get_connection_context() as conn:
                ...
                # Fetch the complete record
                cursor.execute("SELECT * FROM project_versions WHERE id = ?", (last_id,))
                row = cursor.fetchone()
                ...
                if row:
                    ...
                    raise Exception(f"Failed to retrieve inserted project version with ID {last_id}")
