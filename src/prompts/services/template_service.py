...
from ...core.exceptions.base import ConfigurationException, ServiceException
...
class TemplateService(BaseService):
    ...
    def load_template(self, template_name: str) -> ServiceResult[str]:
        ...
        try:
            ...
            if template_name in self._template_cache:
                return ServiceResult(success=True, data=self._template_cache[template_name])
        ...
        try:
            ...
            if self.templates_dir.exists():
                for file_path in self.templates_dir.glob("*.txt"):
                    ...
                    if template_name not in [self.default_templates.get(k, '').replace('.txt', '') 
    ...
    def create_custom_template(
        ...
        try:
            ...
            if not validation_result.data["valid"]:
                return ServiceResult(
                    success=False,
                    error=f"Template validation failed: {', '.join(validation_result.data['issues'])}",
    ...
    def _get_template_path(self, template_name: str) -> Optional[Path]:
        ...
        if template_name == "enhancement" and self.config.external_services.enhancement_template:
    ...
    def _substitute_variables(self, template_content: str, variables: Dict[str, Any]) -> ServiceResult[str]:
        """
