...
try:
    from email.validator import validate_email as email_validator, EmailNotValidError
...
def validate_prompt_name(name: str, max_length: int = 100) -> bool:
    ...
    if len(name) > max_length:
        raise ValidationException(f"Prompt name must be no more than {max_length} characters")
    ...
    if name[0] in '-_' or name[-1] in '-_':
        raise ValidationException("Prompt name cannot start or end with hyphens or underscores")
...
def validate_category(category: str, allowed_categories: Optional[List[str]] = None) -> bool:
    ...
    if allowed_categories is None:
        allowed_categories = [
            'General', 'Writing', 'Analysis', 'Code', 'Creative', 
            'Business', 'Technical', 'Research', 'Education', 'Other'
        ]
...
def validate_tags(tags_string: str, max_tags: int = 10, max_tag_length: int = 30) -> List[str]:
    """
    ...
    for tag in tags:
        if len(tag) > max_tag_length:
            raise ValidationException(f"Tag '{tag}' is too long (max {max_tag_length} characters)")
