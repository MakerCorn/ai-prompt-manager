...
class PromptBuilder:
    ...
    def combine_prompts(self, selected_prompts: List[Dict], template: str = 'sequential', 
                       custom_options: Optional[Dict] = None) -> Tuple[bool, str, Dict]:
        """
    ...
    def _combine_sections(self, prompts: List[Dict], options: Dict) -> str:
        ...
        for prompt in prompts:
            header = f"## {prompt['title']}\n" if prompt.get('title') else f"## {prompt['name']}\n"
    ...
    def _combine_custom(self, prompts: List[Dict], options: Dict) -> str:
        ...
        template = options.get('template', '{content}')
        separator = options.get('separator', '\n\n')
    ...
    def _generate_suggested_name(self, prompts: List[Dict]) -> str:
        ...
        if len(prompts) <= 3:
            ...
            return f"Combined_{len(prompts)}_Prompts_{datetime.now().strftime('%Y%m%d')}"
    ...
    def get_combination_preview(self, selected_prompts: List[Dict], template: str = 'sequential') -> str:
